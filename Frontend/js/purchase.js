...
const checkInputAddress = (input) => {                             // verifie <2 lettre
    if (input.length <= 2) {
...
const checkInputCity = (input) => {                                // verifie <1 lettre et pas de chiffre
    if (input.length <= 1 || input.match(/[0-9]/g)) {
        alert("Ville mal saisi");
        return false;
    }
    return true;
};
...
const buildContactData = () => {
    ...
    if (
        !checkInputName(lastName) ||                               // controle de chaque variable avant envoi du formulaire 
